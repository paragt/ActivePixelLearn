...
void kMeans::compute_centers(std::vector< std::vector<float> >& data, std::vector<unsigned int>& cidx)
{
...
// //     fclose(fp);
    ...
    std::vector< std::vector<float> > ctrs;
    initial_centers(data, ctrs);
    
    unsigned int iter=0; 
    float delta_dist0 = DBL_MAX, delta_dist, delta_diff=DBL_MAX;
...
void kMeans::initial_centers(std::vector< std::vector<float> >& data, std::vector< std::vector<float> >& ctrs)
{
...
float kMeans::kmiter(std::vector< std::vector<float> >& data, std::vector< std::vector<float> >& ctrs,
	      std::vector<unsigned int>& idx_closest_pt)
{
    std::vector< std::vector<float> > ret_ctrs(_K);
    std::vector<float> count(_K, 0);
    
    std::vector<float> dist_closest_pt(_K, DBL_MAX);
