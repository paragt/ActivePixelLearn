...
struct vectorcomp {
    bool operator() (const vector<float>& lhs, const vector<float>& rhs) const
    {
...
class UniqueRowMatrix {
  public:	
    ...
    int insert(const vector<float>& newrow)
    {
    ...
    void get_matrix(vector< vector<float> >& rmatrix)
    {
        rmatrix.clear(); 
        rmatrix.resize(_nrows); 

        set< vector<float>, vectorcomp >::iterator sit;
        int rr=0;
    ...
    void get_vector( vector<float>& rvector)
    {
        rvector.clear(); 
  ...
  protected:
    set< vector<float>, vectorcomp > _rows ;
    size_t _nrows;
    size_t _ncols;	

  
  private:
    set< vector<float>, vectorcomp >& get_set(){return _rows;};	
};
...
public:
    ...
    void get_feature_label(vector< vector<float> >& rmatrix, vector<int>& rlabels){
...
class UniqueRowMatrix_Chull: public UniqueRowMatrix {
  public:
	...
	set< vector<float>, vectorcomp >::iterator sit;
	set< vector<float>, vectorcomp >& another_set = another._rows;
